			...
			<section>
				<div class="d-flex justify-content-between w-100 flex-wrap py-4 px-3">
					<div class="mb-3 mb-lg-0">
						<h1 class="h4">Samples >> Update</h1>
						<p class="mb-0">
							Perbaharui data sample pasien!
						</p>
					</div>
				</div>
			</section>
									...
									<div class="input-group">
										...
										<select id="patient_id" name="patient_id" class="form-select" required>
											<?php foreach ($patients as $patient): ?>
												<option
													value="<?= $patient->id ?>"
													<?= ($patient->id === $sample->patient_id) ? 'selected' : '' ?>
												>
									...
									<div class="input-group">
										...
										<select id="checkup_id" name="checkup_id" class="form-select" required>
											<?php foreach ($checkups as $checkup): ?>
												<option
													value="<?= $checkup->id ?>"
													<?= ($checkup->id === $sample->checkup_id) ? 'selected' : '' ?>
												>
													<?= $checkup->title?>
												</option>
											<?php endforeach; ?>
										...
										<select id="explanation_id" name="explanation_id" class="form-select" required>
											<?php foreach ($explanations as $explanation): ?>
												<option
													...
													<?= ($explanation->id === $sample->explanation_id) ? 'selected' : '' ?>
												>
													<?= $explanation->title?>
												</option>
											<?php endforeach; ?>
